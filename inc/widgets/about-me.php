<?php
add_action('widgets_init', 'about_me_init_child');

function about_me_init_child() {
    register_widget('alia_about_me_child');
}

function widgets_script_child(){
	global $pagenow;
...
class alia_about_me_child extends WP_Widget {
    ...
    function __construct() {
		parent::__construct(
			'about-me-inline-image-widget', // Base ID
			theme_name . ' - ' . __('About Me Inline Image', 'alia-core'), // Name
    ...
    function widget($args, $instance) {
        ...
        foreach ($social_networks as $network => $social ) {
            $$network = isset( $instance[$network] ) ? esc_url($instance[$network]) : '' ;
				...
				$src = 'src=';
				$class = 'alia_about_me_image';
				if (alia_option('alia_lazyload_image_banner', 0) == 1) {
					$src = 'src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src=';
					$class .= ' lazyload';
				}
        ...
        if ($image != '') {
					?>
					<img class="<?php echo $class; ?>" alt="<?php if (isset($title)) { echo $title; } else { echo 'About Me';}?>" <?php echo $src; ?>"<?php echo esc_url($image); ?>" />
					<?php
				}
	        ...
	        foreach ($social_networks as $network => $social ) {
	            if ( $$network != "") {
	                ...
	                $below_bio_text .= '<a rel="nofollow noreferrer" target="_blank" href="'.$$network.'" title="'.$social.'" class="social_icon widget_social_icon social_' . $network . ' social_icon_' . $network . '"><i class="fab fa-' . $network . '"></i></a>';
